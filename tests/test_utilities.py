...
class TestUtilities:
    ...
    def test_default_json_path_creates_directory(self, temp_home, original_cwd):
        """Test that default_json_path creates UESgraphOutput directory"""
        # Call the function
        result_path = default_json_path()
        
        expected_path = temp_home / "UESgraphOutput"
        
        # Verify the directory was created
        assert os.path.exists(result_path), "UESgraphOutput directory should be created"
        assert os.path.isdir(result_path), "UESgraphOutput should be a directory"
        assert result_path == str(expected_path), f"Expected {expected_path}, got {result_path}"
    
    def test_default_json_path_existing_directory(self, temp_home, original_cwd):
        """Test that default_json_path works when directory already exists"""
        # Pre-create the directory
        uesgraph_dir = temp_home / "UESgraphOutput"
        uesgraph_dir.mkdir()
        
        result_path = default_json_path()
        
        # Should return the existing directory
        assert result_path == str(uesgraph_dir)
        assert os.path.exists(result_path)
    
    def test_default_json_path_changes_working_directory(self, temp_home, original_cwd):
        """Test that default_json_path changes to the UESgraphOutput directory"""
        result_path = default_json_path()
        current_dir = os.getcwd()
        
        assert current_dir == result_path, "Should change working directory to UESgraphOutput"
    
    def test_make_workspace_default_name(self, temp_home, original_cwd):
        """Test make_workspace with default 'Project' name"""
        # First create the base directory
        default_json_path()
        
        workspace = make_workspace()
        
        expected_workspace = temp_home / "UESgraphOutput" / "Project"
        assert workspace == str(expected_workspace)
        assert os.path.exists(workspace), "Project workspace should be created"
        assert os.path.isdir(workspace), "Project workspace should be a directory"
    
    def test_make_workspace_custom_name(self, temp_home, original_cwd):
        """Test make_workspace with custom name"""
        # First create the base directory
        default_json_path()
        
        custom_name = "MyTestWorkspace"
        workspace = make_workspace(custom_name)
        
        expected_workspace = temp_home / "UESgraphOutput" / custom_name
        assert workspace == str(expected_workspace)
        assert os.path.exists(workspace), "Custom workspace should be created"
        assert os.path.isdir(workspace), "Custom workspace should be a directory"
    
    def test_make_workspace_existing_workspace(self, temp_home, original_cwd):
        """Test make_workspace when workspace already exists"""
        # First create the base directory
        default_json_path()
        
        # Pre-create workspace
        test_name = "ExistingWorkspace"
        expected_workspace = temp_home / "UESgraphOutput" / test_name
        expected_workspace.mkdir()
        
        workspace = make_workspace(test_name)
