...
def simulate(
    ...
    logger =set_up_terminal_logger("simulate_worker_" + str(process_number))
    ...
    for count, bldg in enumerate(sim_part):
        ...
        if count % SIMULATIONS_BEFORE_RESTART == 0:
            ...
            dymola.openModel(
                path=os.path.join(aixlib_path, "package.mo")
            )
        ...
        logger.info("simulate building {} in progres".format(bldg.name))
        ...
        if output[0] is False:
            logger.info("Simulation failed. Below is the translation log.")
