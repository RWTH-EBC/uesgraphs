  AixLib.Fluid.DistrictHeatingCooling.Demands.ClosedLoop.ValveControlledHeatPumpFixDeltaT demand${str(name)}(
    ...
    %endif
    %if dpValve_nominal is not None:
    dpValve_nominal = ${str(round(dpValve_nominal, 4))},
    %endif
...
<%def name="get_main_parameters()">
   ...
   dp_nominal deltaM tau energyDynamics massDynamics dpValve_nominal sta_default cp_default cp_default_building dTEva_nominal dTCon_nominal m_flow_nominal m_flow_small show_T _m_flow_start _dp_start allowFlowReversal
